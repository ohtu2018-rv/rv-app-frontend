...
export default class ShoppingCart extends React.Component {
  ...
  calculateCartTotal() {
    return placeholderCart.map((item) => item.price * item.quantity)
  ...
  render() {
    ...
    return <div className="shopping-cart">
      ...
      <div className="cart-items-header">
        <Row>
          <Col xs={6} className="cart-item-header">Tuote</Col>
          <Col xs={3} className="cart-item-quantity-header">Määrä</Col>
          <Col xs={3} className="cart-item-total-header">Hinta</Col>
        </Row>
      </div>
      <ul>{cartItems}</ul>
      <div className="cart-total">
        <Row end="xs">
          ...
          {(this.calculateCartTotal() / 100).toFixed(2)} &euro;
          </Col>
        </Row>
      </div>
    </div>;
