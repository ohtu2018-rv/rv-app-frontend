...
describe('RegisterReducer', () => {
    it('action creators return correct actions', () => {
        ...
        expect(checkPasswordsMatch()).toEqual({
            passwordsMatch: true,
            type: 'MARK_PASSWORD_MATCH'
        });
        expect(reset()).toEqual({ type: 'RESET_REGISTER' });
        expect(handleInputEvent({ target: { value: 'e' } })).toEqual({
            target: undefined,
    ...
    it('input is dispatched correctly', () => {
        expect(handleInputEvent({ target: { value: 'test' } })).toEqual({
            target: undefined,
            type: 'INPUT_EVENT_REGISTER',
            value: 'test'
        });
    ...
    describe('Password match', () => {
        it('dispatched false on not equal', () => {
            expect(checkPasswordsMatch('is', 'not is')).toEqual({
        ...
        it('dispatched true on equal', () => {
            expect(checkPasswordsMatch('is', 'is')).toEqual({
                passwordsMatch: true,
                type: 'MARK_PASSWORD_MATCH'
            });
        });
